...
export class CurlHelper {
    ...
    getHeaders() {
      ...
      for(let property in headers) {
        if({}.hasOwnProperty.call(headers, property)) {
          ...
          if(headers[property] == 'application/x-www-form-urlencoded') {
    ...
    getQueryString() {
      ...
      for(let param in this.request.params) {
        ...
        const data = typeof this.request.params[param] == 'object' 
        ? JSON.stringify(this.request.params[param]) :this.request.params[param] 
          params +=
          i !== 0
            ? `&${param}=${data}`
            : `?${param}=${data}`;
